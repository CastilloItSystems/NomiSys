...
import { createNomiSys, updateNomiSys } from "@/app/api/autoSysService";

type FormData = z.infer<typeof autoSysSchema>;

interface NomiSysFormProps {
  autoSys: any;
  hideNomiSysFormDialog: () => void;
  autoSyss: any[];
  setNomiSyss: (autoSyss: any[]) => void;
}

const NomiSysForm = ({
  autoSys,
  hideNomiSysFormDialog,
  autoSyss,
  setNomiSyss,
}: NomiSysFormProps) => {
  ...
  const onSubmit = async (data: FormData) => {
    ...
    try {
      if (autoSys) {
        ...
        const autoSysActualizado = await updateNomiSys(autoSys.id, data);
        ...
        if (index !== -1) {
          ...
          const NomiSyss = [...autoSyss];

          // Actualizar el taller en la copia del arreglo
          NomiSyss[index] = autoSysActualizado;
        ...
        const autoSysCreado = await createNomiSys(data);
