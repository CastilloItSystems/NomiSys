...
// Note: feature-specific SocketService removed; core socket initialized above
...
export const startServer = async (): Promise<void> => {
  try {
    ...
    // Sync roles for all active companies
    const companies = await prisma.company.findMany({
      where: { deleted: false },
      select: { id: true },
    })
    for (const company of companies) {
      await seedDefaultRolesForEmpresa(company.id)
    }
    logger.info(`✅ Roles synced for ${companies.length} company/companies`)
